...
class ImageNetS3(IterableDataset):
    ...
    Constructor has same arguments as base_dataset
    """
    ...
    !!!In results['filename'] below, I am currently not adding the prefix to the
    filename - check that if that is essential.
        - mostly NO - that prefix is just used to laod to the file from disk
    loader.py has an if condition which might be doing this for imagenet

    !!!check what values are passed to the constructor for imagenet in loading.py
    accordingly set the values of to_float32, color_type and rest of the variables
    in result should also be the same
    ...
    def make_pipeline_ready(self, label, image_name, img_bytes, to_float32=False, color_type='color'):
        ...
        results['gt_label'] = np.array(label, dtype=np.int64)
        return results
