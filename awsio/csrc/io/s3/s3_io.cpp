...
namespace {
    ...
    public:
       ...
       //     this->s3_client = initializeS3Client();
            ...
            ShutdownClient(this->s3_client_);
        }
         ...
         //   initializeTransferManager();
            ...
            auto create_stream_fn = [&]() {  // create stream lambda fn
                return Aws::New<S3UnderlyingStream>(
                        "S3ReadStream",
                        Aws::New<Aws::Utils::Stream::PreallocatedStreamBuf>("S3ReadStream", reinterpret_cast<unsigned char*>(buffer), n));
            };
            ...
            std::shared_ptr <Aws::Transfer::TransferHandle> downloadHandle =
                    this->transfer_manager_.get()->DownloadFile(this->bucket_name_.c_str(), this->object_name_.c_str(), offset, n, create_stream_fn);
            ...
            ShutdownClient(this->s3_client_);
	    ShutdownTransferManager(this->transfer_manager_);
